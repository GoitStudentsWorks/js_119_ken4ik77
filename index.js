import{a as d,S as b,N as w,P as L}from"./assets/vendor-DeWL0fjv.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))i(t);new MutationObserver(t=>{for(const s of t)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function n(t){const s={};return t.integrity&&(s.integrity=t.integrity),t.referrerPolicy&&(s.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?s.credentials="include":t.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(t){if(t.ep)return;t.ep=!0;const s=n(t);fetch(t.href,s)}})();const f="/js_119_ken4ik77/assets/icons-CtQiyP8Q.svg";async function g(){const e=document.querySelectorAll(".learn-more"),a=document.querySelector("[data-artist-loader]");e.forEach(n=>{n.addEventListener("click",async()=>{a.hidden=!1;const i=n.dataset.id;try{const s=(await d.get(`/artists/${i}/albums`)).data,o=document.querySelector(".artist-modal"),r=document.querySelector(".artist-info");r.innerHTML=$(s),o.classList.remove("hidden-artist-modal"),document.body.classList.toggle("no-scroll")}catch(t){const s=`Error while executing the request: ${t}`;console.error(s),alert(`❌${s}`)}finally{a.hidden=!0}})})}const c=document.querySelector(".artist-modal"),k=document.querySelector(".close-artist-modal");function u(){c.classList.add("hidden-artist-modal"),document.body.classList.remove("no-scroll")}k.addEventListener("click",u);c.addEventListener("click",e=>{e.target===c&&u()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!c.classList.contains("hidden-artist-modal")&&u()});function $(e){var a,n;return`
                      ...
                      ${s.movie?`
                        <a href="${s.movie}" target="_blank">
                          <svg width="24px" height="24px" class="youtube-modal">
                            <use xlink:href="${f}#icon-Youtube"></use>
                          </svg>
                        </a>`:""}
                    </div>
                  </div>
  ...
  `}async function v(e=1,a=8){return(await d.get("/artists",{params:{page:e,limit:a}})).data}function E(e){var a;return`
    <li class="artist-card">
      ...
      <button class="learn-more" data-id="${e._id}">
        ...
        <svg class="icon-artists-learn-more" width="8" height="24">
          <use xlink:href="${f}#icon-artist-svg-1"></use>
        </svg>
      </button>
    </li>
  `}async function h(e,a,n){const i=document.querySelector("[data-artists-grid]"),t=document.querySelector("[data-artists-load]"),s=document.querySelector("[data-artists-loader]");s.hidden=!1,t.hidden=!0,i.insertAdjacentHTML("beforeend",e.artists.map(E).join("")),s.hidden=!0,a*n<e.totalArtists&&(t.hidden=!1),t==null||t.addEventListener("click",async()=>{a++;const o=await v(a);await h(o,a,n),g()})}(()=>{const e={openModalBtn:document.querySelector("[data-menu-open]"),closeModalBtn:document.querySelector("[data-menu-close]"),modal:document.querySelector("[data-menu]"),menuLinks:document.querySelectorAll(".mobile-nav-style")};function a(){e.modal.classList.toggle("is-open")}e.openModalBtn.addEventListener("click",a),e.closeModalBtn.addEventListener("click",a),e.menuLinks.forEach(n=>{n.addEventListener("click",()=>{e.modal.classList.remove("is-open")})})})();const M=`<svg style="position: absolute; width: 0; height: 0; overflow: hidden;" version="1.1" xmlns="http://www.w3.org/2000/svg"
...
</svg>`;async function S(){return(await d.get("/feedbacks",{params:{limit:10,page:1}})).data}async function q(e){const a=document.querySelector("div.swiper-wrapper"),n=e.map(({name:t,rating:s,descr:o},r)=>`<div class="swiper-slide">
            <div class="feedback-card">
                ${x(s)}
                <p class="review">"${o}"</p>
                <p class="review-author-name">${t}</p>
            </div>
        </div>`).join("");a.insertAdjacentHTML("beforeend",n),B(e.length);const i=new b(".swiper",{modules:[w,L],direction:"horizontal",loop:!0,pagination:{el:".swiper-pagination",type:"custom",renderCustom:function(t,s,o){return A()}},navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"},on:{slideChange:function(){p(this.realIndex+1,e.length)}}});document.querySelector(".swiper-button-next").addEventListener("click",()=>i.slideNext()),document.querySelector(".swiper-button-prev").addEventListener("click",()=>i.slidePrev()),p(1,e.length)}function B(e){const a=document.querySelector(".swiper-pagination");a.innerHTML=`
